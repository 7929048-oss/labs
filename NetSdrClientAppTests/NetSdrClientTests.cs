...
// --- Appended networking and wrapper tests ---
    ...
    public class UdpClientWrapperTests
    {
        ...
        public void EqualsAndGetHashCode_Consistency()
        {
         ...
         // Приклад 1: UI не має залежати від Infrastructure напряму
        [Fact]
        public void UI_Should_Not_Depend_On_Infrastructure()
        {
            ...
            var result = Types.InCurrentDomain()
                .That()
                .ResideInNamespace("NetSdrClient.UI") // заміни на реальні неймспейси
                .ShouldNot()
                .HaveDependencyOn("NetSdrClient.Infrastructure") // заміни на реальні неймспейси
                .GetResult();

            // Assert
            Assert.True(result.IsSuccessful, 
                $"Порушення правила: UI залежить від Infrastructure. \n" +
                $"Типи, що порушують: {string.Join(", ", result.FailingTypes?.Select(t => t.Name) ?? new List<string>())}");
        }

        // Приклад 2: Core не має залежати від UI
        [Fact]
        public void Core_Should_Not_Depend_On_UI()
        {
            var result = Types.InCurrentDomain()
                .That()
                .ResideInNamespace("NetSdrClient.Core")
                .ShouldNot()
                .HaveDependencyOn("NetSdrClient.UI")
                .GetResult();

            Assert.True(result.IsSuccessful, 
                $"Порушення правила: Core залежить від UI");
        }

        // Приклад 3: Всі сервіси мають інтерфейси
        [Fact]
        public void Services_Should_Have_Interface()
        {
            var result = Types.InCurrentDomain()
                .That()
                .ResideInNamespace("NetSdrClient.Services")
                .And()
                .AreClasses()
                .Should()
                .HaveDependencyOn("NetSdrClient.Interfaces")
                .GetResult();

            Assert.True(result.IsSuccessful, 
                $"Сервіси без інтерфейсів");
        }

        // Приклад 4: Контролери мають бути в окремому проекті/неймспейсі
        [Fact]
        public void Controllers_Should_Be_In_API_Project()
        {
            var result = Types.InCurrentDomain()
                .That()
                .HaveNameEndingWith("Controller")
                .Should()
                .ResideInNamespace("NetSdrClient.API")
                .GetResult();

            Assert.True(result.IsSuccessful, 
                $"Контролери знайдені поза API проектом");
        }
    }